...
# Poisson regression
glm_model <- glm(time_at_shelter ~ animal_type +  month_ordered + intake_type +  outcome_type + chip_status,
                 data = animal,
...
full_model <- glm.nb(time_at_shelter ~ animal_type + intake_type + outcome_type  + chip_status + month_ordered , data = animal)
...
nb_model_1 <- glm.nb(time_at_shelter ~ animal_type + month_ordered + intake_type +  chip_status,data = animal)
...
library(ggplot2)
# Residual Plot
model_data <- model.frame(nb_model)
model_data$residuals <- residuals(nb_model, type = "deviance")
predicted_values <- predict(nb_model, type = "response")

ggplot(model_data, aes(x = predicted_values, y = residuals)) +
...
# Comment:
# This plot shows the relationship between the predicted values and the deviance residuals of the nb_model, indicating potential heteroscedasticity.
